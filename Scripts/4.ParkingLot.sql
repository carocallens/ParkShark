...
alter table PL.ParkingLots
	add constraint PhoneNumber_Only_One 
	check ((ContactPerson_PhoneNumber is null and ContactPerson_MobileNumber is not null) or 
		(ContactPerson_PhoneNumber is not null and ContactPerson_MobileNumber is null) or 
		(ContactPerson_PhoneNumber is not null and ContactPerson_MobileNumber is not null))

alter table PL.ParkingLots
	add constraint ParkingLots_Divisions_fk 
	foreign key (Division_ID) references Div.Division(Division_ID)

alter table PL.ParkingLots
	add constraint ParkingLots_Cities_fk 
	foreign key (ParkingLot_City_ZIP) references Mem.Cities(City_ZIP)

alter table PL.ParkingLots
	add constraint ContactPerson_Cities_fk 
	foreign key (ContactPerson_City_ZIP) references Mem.Cities(City_ZIP)